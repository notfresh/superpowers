# Explore Skill — Design Spec

## Overview

A new Superpowers skill that lets an agent **explore a project's code architecture, conventions, and design** through natural dialogue, and **sediment the findings** as Markdown files that grow into a reusable knowledge base over time.

Two sediment files are produced and maintained:

- `INDEX.md` — a dictionary of **explored topics** (agent-generated knowledge)
- `DOCUMENTS.md` — a registry of **discovered project documents** (existing files) with value ratings

The skill is intentionally minimal: a new SKILL.md, a Claude Code slash command, and two empty seed files. It does not modify any existing skill, hook, or harness config.
...
- Cross-harness support beyond Claude Code in this PR
...
```
新增: skills/explore/SKILL.md
新增: commands/explore.md
新增: docs/superpowers/explorations/INDEX.md          (empty template, committed)
新增: docs/superpowers/explorations/DOCUMENTS.md     (empty template, committed)
新增: docs/superpowers/specs/2026-06-13-explore-skill-design.md  (this file)
```

The two seed files under `explorations/` are committed as empty templates so that the first run of the skill always has a predictable starting state (the skill's "if file does not exist, create" path is the only one that runs).

No edits to:
- `skills/*/SKILL.md` (other skills)
- `.claude-plugin/plugin.json` or `marketplace.json`
- `.codex-plugin/`, `.cursor-plugin/`, `.opencode/`
- `hooks/`
- `package.json`
...
Written into the SKILL.md as a hard "never" / "always" list:
...
This skill is **purely additive**. No other skill is required to change. The integration is documented in the new `SKILL.md` and stands on its own:

```markdown
## Integration Points

This skill is purely additive. Other skills can consult its outputs when relevant:
...
Tests to run after implementation, on at least one harness (Claude Code):

| # | Scenario | Pass Criteria |
|---|---|---|
| 1 | `/explore auth` (directed, first run) | File written, INDEX.md has one entry, DOCUMENTS.md has entries for any new docs read |
| 2 | `/explore` (casual, INDEX.md has content) | Agent identifies a gap, proposes candidates, waits for user |
| 3 | `/explore` (casual, INDEX.md empty) | Agent asks "你最想了解这个项目的哪块?" |
| 4 | Same 🟢 document is relevant to two consecutive explorations | First exploration: Read + register in DOCUMENTS.md with impression; second exploration: rely on the cached impression in DOCUMENTS.md and skip the re-Read |
| 5 | Code-vs-doc mismatch is relevant to topic | Exploration file has `## 代码 vs 文档` section |
| 6 | Code-vs-doc mismatch is off-topic | Not reported in any output |
| 7 | 🟠 document is encountered | Agent pauses and asks the user before reading |
| 8 | `/explore` on a non-existent topic (e.g., `/explore unicorn-factory`) | Agent reports "找不到相关代码" gracefully, does not create empty files |

The acceptance test follows the same shape as the harness-integration acceptance test in `CLAUDE.md`: a clean session, send the trigger, observe the right skill firing and the right artifacts appearing.

## Open Questions

None at design time. All known questions resolved during brainstorming:

- Q: Where do the sediment files live? → `docs/superpowers/explorations/`
- Q: How do other skills find them? → Pull-based; `Read INDEX.md` / `Read DOCUMENTS.md` on demand
- Q: What is the tier for each document? → 🟢🟡🔴🟠 four tiers
